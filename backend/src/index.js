...
import { sendVerificationEmail, sendPurchaseConfirmationEmail, verifyEmailConnection } from './services/emailService.js';
import {
  clearProductsCache,
  deleteOtp,
  getOtp,
...
app.post('/api/products', upload.single('image'), async (req, res) => {
  try {
    ...
    const caseMaterialId = await upsertLookup('case_materials', caseMaterial || 'Unknown');
  ...
  try {
    ...
    await query(
      `UPDATE watches SET
       ...
       WHERE id = ?`,
      [
        name, brandId, categoryId, movementId, caseMaterialId,
        parseFloat(price),
        originalPrice != null ? parseFloat(originalPrice) : null,
        imagePath,
        waterResistance, warranty, caseSize, description,
        parseInt(stock, 10),
        parseFloat(rating),
        parseInt(reviews, 10),
...
app.post('/api/auth/register-otp', async (req, res) => {
  ...
  try {
    ...
    await setOtp(`reg_${email}`, { otp, expiresAt, data: { name, email, password, phone, address, city } });
...
app.post('/api/auth/register-verify', async (req, res) => {
  ...
  try {
    ...
    await deleteOtp(`reg_${email}`);
    res.status(201).json({ ...newUser, phone: addr?.phone || '', address: addr?.address || '', city: addr?.city || '' });
  } catch (error) {
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    const [user] = await query('SELECT * FROM users WHERE LOWER(email) = ?', [email]);
    ...
    res.json({ ...cleanUser, phone: addr?.phone || '', address: addr?.address || '', city: addr?.city || '' });
  } catch (error) {
...
app.post('/api/auth/forgot-password-otp', async (req, res) => {
  ...
  try {
    const [user] = await query('SELECT id, email FROM users WHERE LOWER(email) = ?', [email]);
    ...
    const emailResult = await sendVerificationEmail(email, otp, 'passwordReset');
    if (emailResult.success) {
...
app.post('/api/auth/reset-password', async (req, res) => {
  ...
  try {
    await query('UPDATE users SET password = ? WHERE id = ?', [hashPassword(newPassword), pending.userId]);
    await deleteOtp(`reset_${email}`);
...
app.post('/api/orders/request-otp', async (req, res) => {
  ...
  try {
    ...
    const emailResult = await sendVerificationEmail(email, otp, 'orderVerification');
    if (emailResult.success) {
      res.json({ message: 'Verification code sent to your email.' });
    } else {
...
app.post('/api/orders/verify', async (req, res) => {
  ...
  const { userId, total, status, paymentMethod, shippingAddress, items } = orderData;
  const phone = orderData.phone || pending.phone || '';

  if (!userId || !total || !paymentMethod || !shippingAddress || !Array.isArray(items) || !items.length) {
  ...
  try {
    ...
    await query(
      `INSERT INTO orders (id, user_id, address_id, total, status, payment_method, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [orderId, userId, addressId, total, status || 'Pending', paymentMethod, createdAt]
    );
    ...
    const confirmationResult = await sendPurchaseConfirmationEmail(email, {
      orderId, totalAmount: total, itemCount: items.length, shippingAddress, paymentMethod,
    });
    if (!confirmationResult.success) {
      console.warn(`⚠️  Purchase confirmation email failed for order ${orderId}, but order was created.`);
    }
...
app.delete('/api/orders/:id', async (req, res) => {
  ...
  try {
    ...
    if (order.status !== 'Shipped') {
      return res.status(400).json({ error: 'Only shipped orders can be deleted.' });
    }
...
app.post('/api/orders/update-status', async (req, res) => {
  ...
  try {
    ...
    if (result.affectedRows === 0) return res.status(404).json({ error: 'Order not found in database.' });
